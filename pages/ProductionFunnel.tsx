...
const ProductionFunnel: React.FC<ProductionFunnelProps> = ({ 
  ...
  const handleConfirmNewProduct = () => {
    if (newProductName && onAddCatalogItem) {
        const newItemId = `prod_${Date.now()}`;
        ...
        // Auto-select in deal if editing
        if (editDealData) {
            const newProd = { productId: newItemId, name: trimmedName, quantity: 1, price: 0, taxPercent: 0 };
            setEditDealData({ ...editDealData, products: [...(editDealData.products || []), newProd] });
        }
        
        // Auto-select in interaction
        setInteractionProduct(trimmedName);
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] md:h-screen bg-[#f4f7f9] overflow-hidden">
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if(c) {
                                                ...
                                                setEditDealData({
                                                    ...
                                                    contactIds: firstFarm?.contacts?.[0] ? [firstFarm.contacts[0].id] : [],
                                                    contactNames: firstFarm?.contacts?.[0] ? [firstFarm.contacts[0].name] : []
                                                });
                                            }
                                        }}
                                    >
                ...
                <div className="flex flex-col gap-4 w-full md:w-auto md:min-w-[300px] md:border-l md:border-slate-100 md:pl-8">
                    <div className="flex justify-between items-start">
                        <div className="flex flex-col w-full">
                            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Nome da Ocupação</label>
                            <input 
                                ...
                                placeholder="Ex: Ocupação Lote 1..."
                            />
                        </div>
                    ...
                    <div className="flex flex-col">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Reserva Ativa (Total)</p>
      ...
      {isEditingClientModalOpen && editDealData?.clientId && (
        <ClientEditModal 
            ...
            client={clients.find(c => c.id === editDealData.clientId) || { id: '', name: '', type: 'Fazenda', phone: '', farms: [], contacts: [], active: true, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }}
